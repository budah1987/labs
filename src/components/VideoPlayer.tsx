...
export function VideoPlayer({ src }: VideoPlayerProps) {
  ...
  // Uses durationRef (not duration state) so this callback is stable and never
  // causes the ResizeObserver to re-register mid-animation.
  const buildBars = useCallback((w: number) => {
    const svg = svgRef.current;
    if (!svg || durationRef.current <= 0 || w <= 0) return;
  ...
  }, []); // stable — reads durationRef directly

  // Rebuild when duration becomes known — use containerRef (normal flow, has real width)
  useEffect(() => {
    if (duration <= 0 || !containerRef.current) return;
    buildBars(containerRef.current.getBoundingClientRect().width - 32);
  }, [duration, buildBars]);

  // Rebuild on resize — observe containerRef which is in normal flow and reliably
  // reports width changes even inside overflow:hidden animated containers
  useEffect(() => {
    const el = containerRef.current;
    if (!el) return;
    const ro = new ResizeObserver(entries => {
      const w = entries[0].contentRect.width - 32; // subtract 2×16px scrubber margin
    ...
    >
      ...
      <div style={{
        ...
        >
          ...
          >
            <defs>
              <filter id="bar-glow-vp" x="-50%" y="-100%" width="200%" height="300%">
            ...
            <line
              ...
              strokeWidth="1.5"
              strokeLinecap="round"
              style={{ opacity: 0, transition: "opacity 0.1s", filter: "url(#bar-glow-vp)" }}
            />
